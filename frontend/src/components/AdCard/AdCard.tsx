...
export function AdCard({ ad }: Props) {
  ...
  return (
    ...
    >
      {/* Media */}
      <div
        className="aspect-video bg-gray-100 dark:bg-gray-800 flex items-center justify-center relative"
        role="img"
        aria-label={ad.headline ? `Ad image for: ${ad.headline}` : 'Ad creative'}
      >
        {proxyUrl && !imgError ? (
          <img
            src={proxyUrl}
        ...
        {ad.videoUrl && (
          <span
            aria-label={
              mediaUrl
                ? 'Video ad — thumbnail analyzed'
                : 'Video ad — no thumbnail, visual analysis limited'
            }
            title={
              mediaUrl
                ? 'Video ad — thumbnail used as visual proxy for AI analysis'
                : 'Video ad — no thumbnail available; AI analysis based on copy only'
            }
      ...
      <div className="p-3 space-y-1 flex flex-col flex-1">
        ...
        <button
          ...
          aria-label={
            isAnalyzing
              ? 'Analyzing with AI'
              : `Analyze with AI${ad.headline ? ` – ${ad.headline}` : ''}`
          }
